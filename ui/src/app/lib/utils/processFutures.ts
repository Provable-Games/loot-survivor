...
export function beastEncounters(
  ...
  xpList.forEach((xp) => {
    ...
    let { rnd2 } = getRandomness(xp, adventurerEntropy, gameEntropy);
...
export function listAllEncounters(
  ...
  if (!adventurerEntropy || !gameEntropy) {
    return encounters;
  }
...
function recurseEncounters(
  ...
  let nextEncounter = {
    ...getNextEncounter(xp, adventurerEntropy, gameEntropy, hasBeast),
    adventurerLevel: Math.floor(Math.sqrt(xp)),
    xp: xp,
  };
...
export function getNextBigEncounter(
  ...
  let encounter = getNextEncounter(xp, adventurerEntropy, gameEntropy);
  if (encounter.encounter !== "Discovery") {
    return encounter;
  }

  return getNextBigEncounter(lvl, xp + 1, adventurerEntropy, gameEntropy);
}
...
function getNextEncounter(
  ...
  let { rnd1, rnd2 } = getRandomness(xp, adventurerEntropy, gameEntropy);
  const level = BigInt(Math.floor(Math.sqrt(xp)));

  let encounter = Number(rnd1 % BigInt(3));

  if (hasBeast || encounter === 0) {
    return beastEncounter(adventurerEntropy, level, xp, rnd2);
  } else if (encounter === 1) {
    return obstacleEncounter(level, rnd2, xp);
  } else {
...
function getRandomness(
  ...
  if (gameEntropy) {
    params.push(BigInt(gameEntropy));
  }
...
function obstacleEncounter(level: bigint, rnd2: bigint, xp: number): Encounter {
...
function calculateDamage(
  ...
  if (beast) {
    ...
    let weapon_type = weapon.type || weapon.item!;
    let beast_armor = beast.type;
