...
export function syscalls({
  ...
  const spawn = useCallback(
    ...
    [adventurer, calls, gameContract, account]
  );

  const explore = useCallback(
    async (till_beast: boolean) => {
      const exploreTx = {
        contractAddress: gameContract?.address ?? "",
        entrypoint: "explore",
    ...
    [account, calls, gameContract, adventurer]
  );

  const attack = useCallback(
    async (tillDeath: boolean, beastData: Beast) => {
  ...
  const mintLords = useCallback(async () => {
